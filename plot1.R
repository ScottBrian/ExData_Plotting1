...
plotdata <- plotdata[which(!is.na(plotdata$Date) & !is.na(plotdata$Global_active_power)  
                               & dmy(plotdata$Date) >= startDate & dmy(plotdata$Date) <= stopDate),c("Date","Global_active_power")]
